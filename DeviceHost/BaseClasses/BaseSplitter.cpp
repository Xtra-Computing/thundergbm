...
double BaseSplitter::m_lambda;//the weight of the cost of complexity of a tree
...
{
	double nodeWeight = (-m_nodeStat[bufferPos].sum_gd / (m_nodeStat[bufferPos].sum_hess + m_lambda));
...
double BaseSplitter::CalGain(const nodeStat &parent, const nodeStat &r_child, const nodeStat &l_child)
{
	...
	//compute the gain
	double fGain = (l_child.sum_gd * l_child.sum_gd)/(l_child.sum_hess + m_lambda) +
				   (r_child.sum_gd * r_child.sum_gd)/(r_child.sum_hess + m_lambda) -
				   (parent.sum_gd * parent.sum_gd)/(parent.sum_hess + m_lambda);
