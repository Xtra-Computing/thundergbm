...
{
	m_nMaxNumofFloatPointInGPU = MemInfo::GetFreeGPUMem();
...
int StorageManager::PartOfRow(int nInstance, int nDim)
{
	...
	int tempRow = RowInRAM(nDim, nInstance, nInstance);//number of rows CPU can store
	if(tempRow < nNumofInsInGPU)//CPU is the bottleneck
	{
		cout << "GPU can compute " << nNumofInsInGPU << " rows; RAM can only store " << tempRow << endl;
		nNumofInsInGPU = tempRow;
	}
...
//This function should be moved to another class specifically for svm training
/*
