...
int DTGPUMemManager::m_maxTreeDepth = -1;

/**
 * @brief: allocate memory for trees
 */
void DTGPUMemManager::allocMemForTrees(int numofTree, int maxNumofNodePerTree, int maxDepth)
{
	PROCESS_ERROR(numofTree > 0);
	PROCESS_ERROR(maxNumofNodePerTree > 0);
	PROCESS_ERROR(maxDepth > 0);
	...
	TreeNode *pAllTreeNodeHost = new TreeNode[maxNumofNodePerTree];
	cudaMemcpy(m_pAllTree, pAllTreeNodeHost, sizeof(TreeNode) * m_numofTree * maxNumofNodePerTree, cudaMemcpyHostToDevice);
