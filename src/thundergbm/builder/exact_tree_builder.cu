...
//                        device_loop_2d(n_column, columns.csc_col_ptr.device_data(),
//                                [=]__device__(int col_id, int fvid) {
//                            //feature value id -> instance id -> node id
//                            int nid = nid_data[iid_data[fvid]];   // instance_id -> node_id
//                            int pid;
//                            //if this node is leaf node, move it to the end
//                            if (nid < nid_offset)
//                                pid = INT_MAX;//todo negative
//                            else
//                                pid = col_id * n_max_nodes_in_level + nid - nid_offset;  // ?
//                            fvid2pid_data[fvid] = pid;
//                        }, n_block);
//                        cudaDeviceSynchronize();
//                        LOG(INFO) << "fvid2pid " << fvid2pid;
//                    }
//
...
//                        LOG(INFO) << "sorted fvid2pid " << fvid2pid;
//                        LOG(INFO) << "fvid_new2old " << fvid_new2old;
//                    }
//                    cudaDeviceSynchronize();
//                }
//
...
//                    auto key_iter = make_zip_iterator(   // tuple(int, float) -- (pid, f_value)
//                            make_tuple(
//                                    fvid2pid.device_data(),
//                                    make_permutation_iterator(
//                                            columns.csc_val.device_data(),
//                                            fvid_new2old.device_data())));//use fvid_new2old to access csc_val
//                    // TODO do not to reduce
//                    n_split = reduce_by_key(   // reduce if pid && f_value are the same
//                            cuda::par,
...
//                    ).first - rle_key.device_data();   // rle_key: (pid, f_value), gh_prefix_sum: reduced gh sum
...
//                            cuda::par,
//                            rle_pid_data, rle_pid_data + n_split,   // rle_pid --->> to get the [0] dimension of tuple 'rle_key'
//                            gh_prefix_sum.device_data(),
//                            gh_prefix_sum.device_data());
//                    LOG(DEBUG) << "gh prefix sum = " << gh_prefix_sum;   // the same as segmented reduce as pids are sorted
//                    cudaDeviceSynchronize();
//                }
//            }
//        }
...
void ExactTreeBuilder::find_split(int level, int device_id) {
    ...
    int nnz = columns.nnz;
    int n_block = std::min((nnz / n_column - 1) / 256 + 1, 32 * 56);
    SyncArray<bool> is_dulipated(nnz);
    ...
    //find the best split locally
    {
        ...
        SyncArray<int_float> rle_key(nnz);
        if (nnz * 4 > 1.5 * (1 << 30)){
            CHECK_EQ(1, 2) << "Unexpected";
            rle_key.resize(int(nnz * 0.1));
        }
//        LOG(INFO)
        auto rle_pid_data = make_transform_iterator(rle_key.device_data(),
                                                    [=]__device__(int_float key) { return get<0>(key); });
                                                     ...
                                                     [=]__device__(int_float key) { return get<1>(key); });
        {
            //gather g/h pairs and do prefix sum
            {
                ...
                //do prefix sum
                {
                    ...
                    auto fvid_new2old_data = fvid_new2old.device_data();

                    // set gradient of duplicated feature values to 0 & set the flag to 1
                    auto is_dulipated_data = is_dulipated.device_data();
                    device_loop(nnz, [=] __device__(int i){
                        int pid = get<0>(key_iter[i]);
                        float_type fvalue = get<1>(key_iter[i]);
                        int fvid = fvid_new2old_data[i];
                         // printf("%d|%f ", pid, fvalue);
                        if(nnz - 1 > i) {
                            int current_pid = get<0>(key_iter[i + 1]);
                            float_type current_fvalue = get<1>(key_iter[i + 1]);
                            if((current_pid == pid) && (current_fvalue == fvalue))
                                is_dulipated_data[i] = 1;
                        }
                        int iid = csc_row_idx_data[fvid];
                        //gh_prefix_sum_data[fvid] = gh_pair_data[iid];
                        gh_prefix_sum_data[i] = gh_pair_data[iid];
                    });

                    // init rle for less refactor
                    auto rle_key_data = rle_key.device_data();
                    device_loop(n_split, [=] __device__(int i){
                        get<0>(rle_key_data[i]) = get<0>(key_iter[i]);
                        get<1>(rle_key_data[i]) = get<1>(key_iter[i]);
                    });
                    ...
                    inclusive_scan_by_key(
                            ...
                            gh_prefix_sum.device_data(),
                            gh_prefix_sum.device_data());
                    LOG(DEBUG) << "gh prefix sum = " << gh_prefix_sum;   // the same as segmented reduce as pids were sorted
                    cudaDeviceSynchronize();
                }
            }
        }
        ...
        //calculate missing value for each partition
        {
            ...
            float_type rt_eps = param.rt_eps;
            {
                ...
                auto is_duplicated_data = is_dulipated.device_data();
                device_loop(n_split, [=]__device__(int i) {
                    int pid = rle_pid_data[i];
                    if (pid == INT_MAX) return;
                    float_type f = fval_data[i];
        ...
        // -------------------------------------------------------------
        auto is_duplicated_data = is_dulipated.device_data();
        auto gain_data = gain.device_data();
        device_loop(n_split, [=] __device__(int i) {
            if(is_duplicated_data[i] == 1)
                gain_data[i] = 0;
        });
